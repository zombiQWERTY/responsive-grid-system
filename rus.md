...
сделанная под себя отзывчивая сетка. Вы можете настроить 
всё, что вам нужно, включая количество колонок, размер колонок и отступов
и даже контрольные точки, в которых вы перестраиваете вашу раскладку.

К сожалению, многие люди даже не пытаются делать сетки для своих сайтов,
потому что у них не хватает знаний и уверенности в своих силах.

Итак, в этой статье я хочу помочь вам получить знания и веру в себя, которые 
вам понадобятся для создания собственной сетки. Надеюсь, что к концу этой статьи
вы сможете оторваться от фреймворков и попробовать собственную сетку в следующем
проекте.
...
Вы используете колонки одинаковой или разной ширины? Сколько у вас есть колонок?
Какого размера отступы и колонки?

Вы сможете правильно просчитать параметры сетки только после того, как дадите ответы
на все эти вопросы. Чтобы помочь вам, я написал статью о [проектировании сеток][1].
...
Будете ли вы менять размеры колонок и отступов пропорционально ширине экрана? Будете
ли вы менять только размер колонок, оставляя отступы фиксированными? Будете ли вы менять
количество колонок в конкретных контрольных точках?

На эти вопросы также нужно ответить. Они подскажут вам, как рассчитать ширину колонок
и отступов. Обо всём этом вы можете прочесть в той же [статье о проектировании сеток][2],
если у вас есть сомнения.

**В-третьих, нравится ли вам писать классы сетки в разметке?**
...
Вам нужно меньше разметки тех вещей, которые вы делаете. Вам также не нужно
...
Многие фронтенд-эксперты выбирают CSS-сетки. Я тоже принадлежу фракции CSS-сеток
(несмотря на то, что я не смею называть себя экспертом).

Я написал о том, почему я выбрал CSS-сетки вместо HTML-сеток
...
В любом случае, это те три вещи, которые вам нужно знать, прежде чем 
вы сможете сделать вашу сетку. Вкратце, вот они: 

1.  Дизайн вашей сетки
2.  Как ваша сетка ведет себя на разных вьюпортах
3.  Следует ли использовать CSS-сетки или HTML-сетки

Мы двинемся дальше, только если у нас есть эти компоненты. Далее в статье,
...
используется браузерами для рассчета свойств `width` и `height`. Выставляя `box-sizing`
в `border-box`, мы сильно упрощаем рассчет размеров колонок и отступов
(вы увидите это позже).

Вот картинка, которая демонстрирует как `width` вычисляется в зависимости от значений
`box-sizing`<figure>

![Свойство Box-sizing и как оно влияет на рассчет ширины][6]<figcaption>
Свойство Box-sizing и как оно влияет на рассчет ширины</figcaption></figure>
...
`em` или `rem` вместо пикселов для дотступности и отзывчивости. В этой статье
я пишу все в пикселах, потому что они проще для понимания.


## Шаг 4: Расcчитайте ширину колонок {#step-4-calculate-column-width}
