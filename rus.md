<article>
Одним из лучших инструментов для создания уникального дизайна является 
уникальная отзывчивая сетка. Вы можете настроить 
всё, что вам нужно: количество, ширину и отступы колонок
и даже брейкпоинты, в которых вы перестраиваете вашу сетку.
...
**В-третьих, задумайтесь, нравится ли вам писать классы сетки в разметке?**

Когда дело доходит до типографских сеток, мир фронтенда делится на две фракции.

Одни пишут классы в разметке (такой подход используется, например,
...
*HTML для CSS-сеток проще* чем HTML для *HTML-сеток.*
Вам нужно меньше разметки для того, что вы делаете. Вам также не нужно
...
С другой стороны, *CSS для CSS-сеток сложнее*. 
Нужно хорошенько подумать даже для реализации простой задачи 
(если раньше вы таких сеток не создавали).
 
**Что бы я выбрал?**

Я, как и многие фронтенд-эксперты выбираю CSS-сетки 
(несмотря на то, что я не смею называть себя экспертом).

Я написал [статью][3] о том, почему я выбрал CSS-сетки вместо HTML-сеток,
если вам интересно это выяснить. Еще я написал [статью][4],
...
2.  Как ваша сетка ведет себя на разных эеранах
...
2.  Когда экран меняет размеры, колонки должны менять размеры пропорционально,
